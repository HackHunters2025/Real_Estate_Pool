...
export async function fetchESGStatus(propertyId = "Green Valley Tower") {
  const res = await fetch(`${BASE_URL}/esg/check`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    ...
    body: JSON.stringify({
      property: propertyId,
      energy_usage: 72, // test value (can be dynamic later)
    }),
  });

  if (!res.ok) {
    throw new Error("ESG API failed");
  }

  const data = await res.json();

  console.log("✅ RAW ESG BACKEND RESPONSE:", data);

  const compliance = data.compliance;

  if (!compliance) {
    throw new Error("Invalid ESG response structure");
  }

  return {
    status: compliance.status,            // PASS | FAIL
    message: compliance.details,          // text
    allowedLimit: compliance.allowed_limit_kwh,
  };
}
